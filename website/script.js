...
function fetchParameters() {
    ...
    getParamNamesClient.callService(new ROSLIB.ServiceRequest({}), function(result) {
        console.log(result);
        ...
        paramNames.forEach(function(paramName) {
            ...
            param.get(function(value) {
                ...
                if (fetchedCount === paramNames.length) {
                    ...
                    document.getElementById('params-display').innerHTML = `<pre>${formattedParams}</pre>`;
                }
            });
        });
    });
...
axisTopic.subscribe(function(message) {
    ...
    if (Array.isArray(message.data)) {
        const axisNames = ['Left X', 'Left Y', 'Right X', 'Right Y', 'LT', 'RT', 'DPad X', 'DPad Y'];
        let formattedData = '';
...
buttonTopic.subscribe(function(message) {
    ...
    if (Array.isArray(message.data)) {
        ...
        for (let i = 0; i < message.data.length; i++) {
            const buttonName = i < buttonNames.length ? buttonNames[i] : `Button ${i}`;
...
window.addEventListener("gamepaddisconnected", function(e) {
    ...
    document.getElementById('controller-status-text').textContent = 'Disconnected';
});
...
function readControllerData() {
    ...
    // Check axes
    if (Math.abs(gamepad_axis[0]) > 10) activeControllerElements.add('left-stick-x');
    if (Math.abs(gamepad_axis[1]) > 10) activeControllerElements.add('left-stick-y');
    if (Math.abs(gamepad_axis[2]) > 10) activeControllerElements.add('right-stick-x');
    if (Math.abs(gamepad_axis[3]) > 10) activeControllerElements.add('right-stick-y');
    if (Math.abs(gamepad_axis[6]) > 0) activeControllerElements.add(gamepad_axis[6] < 0 ? 'dpad-left' : 'dpad-right');
    if (Math.abs(gamepad_axis[7]) > 0) activeControllerElements.add(gamepad_axis[7] < 0 ? 'dpad-up' : 'dpad-down');
